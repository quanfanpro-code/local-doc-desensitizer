...
def 固定规则识别(块, 启用日期=False, 启用月日=False, 启用金额=False):
    ...
    for m in re.finditer(r"(?<![0-9A-Za-z])\d(?:[ -]?\d){12,29}(?![0-9A-Za-z])",文本):
        v = re.sub(r"\D","",m.group())
        if (银行卡校验(v) and not 标签(m.start(),r"物料编号|产品编号|商品编码")) or 标签(m.start(),r"银行账[号户]|收款账[号户]|付款账[号户]|开户账号|卡号"):
    ...
    if 启用月日 and 启用日期:
        for m in re.finditer(r"(?<!\d)\d{1,2}月\d{1,2}日",文本):
            记录(m.start(),m.end(),"date",4)
    if 启用金额:
    ...
    for 优先,m in sorted(命中,key=lambda x:(x[0],-(x[1].结束-x[1].开始))):
        if m.类型 == "amount":
            if 块.原文[m.结束:m.结束+1] == "余":
                continue
            m.替换值 = 金额模糊值(unicodedata.normalize("NFKC",m.原词))
            if m.替换值 is None:
                continue
